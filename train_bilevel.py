import torch
from Bird_dataset import Bird_dataset
from torchvision import transforms
from torch.utils.data import DataLoader
from predict_model import Prediction_model
from auxilearn.hypernet import MonoJoint, MonoNonlinearHyperNet
...
parser.add_argument('--method', type = str, default = "joint", help="joint or only task")
...
def train_model(model,epochs,train_loader,val_loader,opt,criterion,device,task_num):
    ...
    for epoch in range(epochs):
        ...
        for data in train_loader:
            ...
            #exit(0)
            main_task_logits = logits[0]
            total_loss = criterion(main_task_logits,label)
            total_loss = total_loss.mean()
            #print(total_loss)
            #exit(0)
            opt.zero_grad()
            for m in range(1,task_num):
                total_loss += args.aux_weight*criterion(logits[m],auxlabel[m-1]).mean() 
            total_loss.backward()
            opt.step()
            running_loss = total_loss.item()/len(label)                         
            _,predictions = torch.max(main_task_logits,1)
            correct_num = torch.sum(predictions == label) + 0.0
            total_num = len(label)
            #exit(0)
            if ite % 20 == 0:
                print("epoch",epoch,"ite:",ite,"training loss:",running_loss,"training acc:",correct_num/total_num)
                logger.info("epoch:%d ,iteration:%d, training loss:%.6f, training acc:%.6f" %(epoch,ite,running_loss,correct_num/total_num) )
            ite += 1
...
def train_meta_model(p_model, aux_model, epochs, train_loader, val_loader, aux_loader, p_opt, aux_opt, criterion,device, task_num, hyperstep):
    ...
    for epoch in range(epochs):
        ...
        for data in train_loader:
            ...
            total_loss = aux_model(loss_vector)
            total_loss.backward()
            p_opt.step()
            ...
            if (counter+1) % hyperstep == 0:
                ...
                for n_val_step, aux_data in enumerate(aux_loader):
                    if n_val_step < args.n_meta_loss_accum:
                        aux_picture, aux_label, aux_auxlabel = aux_data
...
def train_joint_meta_model(p_model, aux_model, epochs, train_loader, val_loader, aux_loader, p_opt, aux_opt, criterion,device, task_num, hyperstep):
    p_model = p_model.to(device)
    aux_model = aux_model.to(device)
    p_model.train()
    aux_model.train()
    ...
    for epoch in range(epochs):
        ...
        for data in train_loader:
            ...
            for m in range(1,task_num):
                auxlabel[m-1] = auxlabel[m-1].to(device)
            #auxlabel = auxlabel.to(device)
            logits,feature = p_model(picture)
            p_opt.zero_grad()
            main_task_logits = logits[0]
            loss_list = []
            main_loss = criterion(main_task_logits,label)
            ...
            for m in range(1,task_num):
                loss_list.append(args.aux_weight*criterion(logits[m],auxlabel[m-1]))
            loss_vector = obtain_loss_vector(loss_list)
            total_loss = aux_model(feature, loss_vector, to_train= True)
            #print("total_loss:", total_loss)
            #print("model_param:", aux_model.task_prior)
            total_loss.backward()
            p_opt.step()
            ...
            if (counter+1) % hyperstep == 0:
                ...
                for n_train_step, data in enumerate(train_loader):
                    if n_train_step < args.n_meta_train_loss_accum:
                        #print("used batch here:",n_train_step)
                        picture, label, auxlabel = data
                        picture = picture.to(device)
                        label = label.to(device)
                        for m in range(1,task_num):
                            auxlabel[m-1] = auxlabel[m-1].to(device)
                        #auxlabel = auxlabel.to(device)
                        logits, features = p_model(picture)
                        main_task_logits = logits[0]
                        loss_list = []
                        main_loss = criterion(main_task_logits,label)
                        loss_list.append(main_loss)
                        for m in range(1,task_num):
                            loss_list.append(args.aux_weight*criterion(logits[m],auxlabel[m-1]))
                        loss_vector = obtain_loss_vector(loss_list)
                        total_loss = aux_model(features,loss_vector,  to_train=False)
                        total_meta_train_loss += total_loss
                    else:
                        break
                #exit(0)
            ...
            counter += 1
        acc = evaluate_model(p_model,val_loader,device)
        #teacher_confidence = min( teacher_confidence * 1.5, 1 )
        # if epoch >= 10:
...
if args.method == "common":
    train_image_file = './preprocess_data/full_train_set.json'
else:
...
train_dataset = Bird_dataset(train_image_file,label_file,image_root,transform=train_transform,finegrain=args.whether_aux,corrupted=args.corupted, cor_rate= args.corupted_rate, aux_file=auxilary_image_file)
train_loader = DataLoader(train_dataset, batch_size=args.train_batch_size, shuffle=True,drop_last=False)

valid_dataset = Bird_dataset(valid_image_file,label_file,image_root,transform=test_transform,finegrain=False)
valid_loader = DataLoader(valid_dataset, batch_size=args.test_batch_size, shuffle=False,drop_last=False)

test_dataset = Bird_dataset(test_image_file,label_file,image_root,transform=test_transform,finegrain=False)
test_loader = DataLoader(test_dataset, batch_size=args.test_batch_size, shuffle=False,drop_last=False)

aux_dataset = Bird_dataset(auxilary_image_file,label_file,image_root,transform=test_transform,finegrain=True)
...
if args.method == "joint":
    aux_model = MonoJoint(main_task=0, input_dim = task_num,feature_dim=512, hidden_sizes= args.aux_hidden_dim, weight_normalization=False, init_lower= 0.0, init_upper=1.0)
else:
    aux_model = MonoNonlinearHyperNet(main_task=0, input_dim = task_num, hidden_sizes= args.aux_hidden_dim, weight_normalization=False, init_lower= 0.0, init_upper=1.0)
...
if args.method == "joint":
    ...
    train_joint_meta_model(p_model, aux_model, epochs, train_loader, valid_loader, aux_loader, optimizer_main, meta_optimizer, crit, device, task_num, hyperstep)
    ...
    train_model(p_model,epochs,train_loader,valid_loader,optimizer_main,crit,device,task_num)
